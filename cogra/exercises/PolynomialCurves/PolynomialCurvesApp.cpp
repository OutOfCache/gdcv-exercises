...
private:
    ...
    gmca::TransformationController2D    m_transformationController;
...
public:
    ...
    void onDraw() override
    {
        ...
        const auto pixelScale = (2.0f / std::min(getFramebufferWidth(), getFramebufferHeight())) / getScaleFactor();
    ...
    void onDrawUI() override
    {
        ...
        ImGui::Begin("Options");
        {
            if(ImGui::CollapsingHeader("Curve"))
            {
                ...
                {
                    if(m_curve->getDegree() + 1 <= gmca::PolynomialCurveSegment::maxDegree)
                    {
                ...
                {
                    if(m_curve->getDegree() - 1 >= gmca::PolynomialCurveSegment::minDegree)
                    {
...
int main()
{
    try
    {
        ...
        GLFWWindow<gmca::PolynomialCurvesApp> app(c);
        app.run();
    }
