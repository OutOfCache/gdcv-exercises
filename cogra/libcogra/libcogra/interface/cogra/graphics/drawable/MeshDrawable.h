...
public:
    ...
    {
        MeshDrawable_impl(indexBuffer, nIndices, nVertices, VertexAttributeTypeInfo(args)...);
    }
    ...
    MeshDrawable(        
        ...
        , m_primitiveType(convertToGLenum(primtiveType))
    {
        MeshDrawable_impl(nullptr, 0, nVertices, VertexAttributeTypeInfo(args)...);
    }
...
private:
    ...
    void MeshDrawable_impl(
        const uint32* const indexBuffer, size_t nIndices, size_t nVertices,
        const Targs&... args)
    {
    ...
    void MeshDrawable_impl(const uint32* const indexBuffer, size_t nIndices, size_t nVertices, const std::vector<VertexAttributeTypeInfo>& vertexAttributeInfo);
